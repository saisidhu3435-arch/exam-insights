...
export function Layout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-[100dvh] flex flex-col max-w-md mx-auto w-full bg-background border-x border-border shadow-2xl relative">
      <header className="sticky top-0 z-50 bg-background/80 backdrop-blur-md border-b border-border px-4 py-3 flex items-center justify-center">
        <Link href="/" className="flex items-center gap-2 hover:opacity-80 transition-opacity">
          <img src={logoImg} alt="Minute Ahead" className="h-6 w-auto object-contain" />
          <span className="font-bold text-lg tracking-tight">Minute Ahead</span>
        </Link>
      </header>
      ...
      <nav className="fixed bottom-0 w-full max-w-md bg-background border-t border-border px-6 py-3 flex items-center justify-between pb-safe z-50">
